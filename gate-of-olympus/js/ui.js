...
export const displayPayouts = (elements, payouts) => {
  elements.payoutsInfo.innerHTML = ''; // kosongkan dulu kontainernya

  for (const symbolPath in payouts) {
    const payoutValue = payouts[symbolPath];

    const row = document.createElement('div');
    row.className = 'flex justify-between items-center py-4 gap-6';
    ...
    img.src = symbolPath;
    img.alt = 'symbol';
    img.className = 'w-[80px] h-[80px] object-contain border-white'; // Ukuran benar + scalable
    symbolDiv.appendChild(img);

    // Nilai payout
    const payoutDiv = document.createElement('div');
    payoutDiv.textContent = payoutValue;
    payoutDiv.className = 'text-2xl font-semibold';

    // Susun row
    row.appendChild(symbolDiv);
    row.appendChild(payoutDiv);
    elements.payoutsInfo.appendChild(row);
  }
};
